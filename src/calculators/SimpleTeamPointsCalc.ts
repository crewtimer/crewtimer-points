...
import { getFinalLevel, numSeatsFromName } from "../common/CrewTimerUtils";
...
export const simplePointsCalc = (resultData: Results): SimplePointsResult => {
    ...
    resultData.results.forEach(eventResult => {
        if (getFinalLevel(eventResult.Event, eventResult.EventNum)) {
    ...
    const sorted = Array.from(teamPoints.entries()).sort((a, b) => b[1] - a[1]).map(v => ({ team: v[0], points: v[1] }));
    return sorted;
}